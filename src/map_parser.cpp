...
void parse_map(const char* filePath, glm::vec3* playerOrig)
{
    ...
    std::vector<MeshStruct> sortedMeshes;
                ...
                if (worldspawn_entity || func_group) {
                    ...
                    if (brushStart) {
                        ...
                        if (line != "}" && line != "{") {
                            ...
                            brushFace.texInfo.width = sortedMeshes[brushFace.texInfo.textureIndex].img.width;
                            brushFace.texInfo.height = sortedMeshes[brushFace.texInfo.textureIndex].img.height;
    ...
    write_bin_file(sortedMeshes);

}

/*
* Write structure visualization for one mesh
struct fileStruct {
...
void write_bin_file(std::vector<MeshStruct> meshStructs)
{
    ...
    map_out.open("C:\\Users\\jp-om\\Documents\\repos\\fpsgl\\resources\\map.dat", std::ios::binary | std::ios::out);
    int meshCount = meshStructs.size();
    map_out.write(reinterpret_cast<const char*>(&meshCount), sizeof(meshCount));

    for (int meshInd = 0; meshInd < meshCount; meshInd++) {
        int vertexCount = meshStructs[meshInd].vertices.size();
        map_out.write(reinterpret_cast<const char*>(&vertexCount), sizeof(vertexCount));
        for (int vertInd = 0; vertInd < vertexCount; vertInd++) {
            map_out.write(reinterpret_cast<const char*>(&meshStructs[meshInd].vertices[vertInd]), sizeof(meshStructs[meshInd].vertices[vertInd]));
            map_out.write(reinterpret_cast<const char*>(&meshStructs[meshInd].texCoords[vertInd]), sizeof(meshStructs[meshInd].texCoords[vertInd]));
        }

        std::ifstream texFile;
        std::string imgPath = globTexPath + meshStructs[meshInd].name + ".png";
        texFile.open(imgPath, std::ios::binary | std::ios::in | std::ios::ate);
        int texFileSize = texFile.tellg();
        texFile.seekg(0, std::ios::beg);

        map_out.write(reinterpret_cast<const char*>(&texFileSize), sizeof(texFileSize));
        std::cout << texFileSize << std::endl;

        for (int i = 0; i < texFileSize; i++) {
            char b;
            texFile.read(reinterpret_cast<char*>(&b), sizeof(b));
            map_out.write(reinterpret_cast<const char*>(&b), sizeof(b));
...
int get_texture_index(std::vector<std::string>& texNames, std::vector<MeshStruct>& sortedMeshes, std::string textureName) {
    ...
    img.data = stbi_load(imgPath.data(), &img.width, &img.height, &img.nrChannels, 0);
    if (!img.data) {
    ...
    newMeshStruct.img = img;
    newMeshStruct.name = textureName;

    sortedMeshes.push_back(newMeshStruct);
